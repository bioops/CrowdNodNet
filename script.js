...
function drawGraph() {
    ...
    for (var name in graph.data) {
        var obj = graph.data[name];
        for (var depIndex in obj.upstream) {
            var link = {
                source : graph.data[obj.upstream[depIndex]],
                target : obj
            };
...
function getDoc(gene){
    var name = gene.name;
    var type = gene.type;

    var doc  = "<h1 class=\"title\">" + gene.name + "</h1>\n<div>" + gene.type + "</div>\n";
    var doc_no = "<div class=\"alert alert-warning\"><a href=\"" + config.wiki + "/" + gene.name +
                 "?action=edit\" target=\"_blank\" data-name=\"$name\">Create annotation</a></div>";

    var doc_yes = "<h1 class=\"section\">Annotation</h1>\n";
    doc_yes += "<a href=\"" + config.wiki + "/" + gene.name + "\" target=\"_blank\" data-name=\"" + gene.name + "\">edit</a>\n";
    var filename = config.wiki + "/" + gene.name + "?action=render";

    $.get(filename)
     .success(function(data) { $("#docs").html(doc + doc_yes + data)})
     .error(function() {$("#docs").html(doc + doc_no)});
    ...
    if (obj) {
        if (obj !== highlighted) {
            graph.node.classed('inactive', function(d) {
                return (obj !== d
                     && d.upstream.indexOf(obj.name) == -1
                     && d.downstream.indexOf(obj.name) == -1);
            });
